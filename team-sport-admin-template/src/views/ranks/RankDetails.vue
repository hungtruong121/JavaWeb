                ...
                <div class="card-body">
                    ...
                    <form @submit="checkFormCreate" method="post">
                        ...
                        <div class="row row-space">
                            <div class="col-1"></div>
                            <div class="col-2">
                                <label class="label">Rank ID</label>
                            </div>
                            <div class="col-9">
                                <CInput
                                    v-model="teamRankId"
                        ...
                        <div class="row row-space">
                            ...
                            <div class="row row-space m-t-5 pb-3" style="display: none;">
                                <div class="col-3">
                                    <div class="input-group">
                                        <CInput
                                ...
                                <div class="col-3">
                                    <div class="input-group">
                                        <CInput
                                            ...
                                            ref="price"
                                            placeholder="Enter Price"
                                            invalid-feedback="Price must have at least 2 and max 9 number characters."
                        ...
                        <div class="row row-space"> 
                            <div class="col-3">
                                <label class="label">Time Modify</label> 
                            </div>
                            <div class="col-9"></div>
                        </div>
                        <div class="row row-space">
...
export default {
    ...
    methods: {
        ...
        checkFormCreate(e) {
            ...
            if (!this.validatorTeamRankDescription(this.teamRankDescription))
             ...
             if (this.sectionTeamRankPackage.length == 0)
                return this.showAlertError("Please add a Packace Price!");
        ...
        addRankPackage() { 
            ...
            if (this.teamRankPackagePrices.rankPackagePriceValue.length >= 9 || this.teamRankPackagePrices.rankPackagePriceValue.length <= 1){
                return this.msgRankPackageErr = "Price must have at least 1 characters and a maximum of 9 characters";
            }
            if (this.teamRankPackagePrices.rankPackagePriceTime.length >= 2 || this.teamRankPackagePrices.rankPackagePriceTime.length <= 1){
                return this.msgRankPackageErr = "Duration must have at least 1 characters and a maximum of 2 characters";
            }
            ...
            if (isExisted){
                return this.msgRankPackageErr = "This Team Package already exists, please input add new rank package!";
            } else {
        ...
        convertEntityToDTO(e){
            var rankName = "";
            this.listRankPackgePrice.forEach(item => {
                if (e.teamRankId == item.teamRankId){
                    rankName = item.teamRankName;
                }   
            });
            this.avatarId = e.teamRankAvatar;
            this.teamRankId = e.teamRankId;
            this.teamRankName = e.teamRankName;
            this.storageCapacity = e.storageCapacity,
            this.teamRankMemberLimit = e.teamRankMemberLimit,
            this.teamRankDescription = e.teamRankDescription,
            this.createdDate = this.formatDate(e.createdDate, "-"),
            this.updatedDate = this.formatDate(e.updatedDate, "-")
        },
        ...
        getRankPackage(){
            this.waitTeamRankDTO = setInterval(() => {
                if (this.teamRankId != "") {
                    clearInterval(this.waitTeamRankDTO);
                    
                    // Get rank package price by RankID
                    axios.get("/admin/rank/listrankpackage?teamRankId=" + this.teamRankId)
                    .then(response => {
                        if (response.data != "") {
                            if (response.data.success){
                                var listRankPackage = response.data.data;
                                listRankPackage.forEach(e => {
                                    var teamRankDTO = { 
                                        rankPackagePriceValue: e.rankPackagePriceValue.toString(),
                                        rankPackagePriceUnit: e.rankPackagePriceUnit,
                                        rankPackagePriceTime: e.rankPackagePriceTime.toString(),
                                    }
                                    this.sectionTeamRankPackage.push(Object.assign({}, teamRankDTO));
                                });
                            } 
                        } else {
    ...
    created() {
        ...
        let ranksDetail = JSON.parse(localStorage.getItem('ranksDetail'));

        // Get rank in items with id = teamRankId
        var rankDetail = {};
        if (ranksDetail && ranksDetail != null) {
            ranksDetail.forEach((item) => {
                rankDetail = item.teamRankId == paramId ? item : rankDetail;
            });

            if (rankDetail.teamRankId && rankDetail.teamRankId != null){
                this.convertEntityToDTO(rankDetail);
                // Load rank avatar
                var baseURL = axios.defaults.baseURL;
                var teamRankAvatar = rankDetail.teamRankAvatar == null ? 1 : rankDetail.teamRankAvatar;
                this.avatar = baseURL + '/file/reads?uploadId=' + teamRankAvatar;
            } else {
                this.showAlertError("Rank ID " + paramId + " doesn't exist!");
            }
        } else {
            this.$router.push({ path: `/rank/listranks`});
        }
